...
class _AddRekamMedisPageState extends State<AddRekamMedisPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<RekamMedisBloc, RekamMedisState>(
        listener: (context, state) {
          if (state is RekamMedisActionSuccess) {
            // Update local medicine list/stock immediately on success
            context.read<MedicineBloc>().add(LoadMedicinesEvent());
            _triggerSuccessNotification();
            Navigator.pop(context);
          } else if (state is RekamMedisError) {
            _showErrorDialog(state.message);
          }
        },
        builder: (context, state) {
          final isSaving = state is RekamMedisLoading;
